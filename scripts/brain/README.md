...
| `site.py` | Renders every documentation file into one self-contained browsable page, `docs/reference.html` (`docs/index.html` is the hand-written landing page). Pure standard library; the markdown subset is deliberately limited so anything outside it renders as plain text rather than silently wrong. |
